...
# acceptable to you.
# ────────────────────────────────────────────────────────────────
RATES_SHEET_ID = "1KiOhZHZHPbWUUVfSUqXy0Y_nsRljSrMcxBT9i2ETjoc"
RATES_SHEET_GID = "466198263"  # confirmed from the rates tab's URL
ROSTER_SHEET_GID = "1211118080"  # confirmed from the Roster tab's URL
...
# P&L sheet — confirmed to match the SHEET_ID hardcoded in index.html, so this
# is the actual sheet the dashboard reads from, not a different one.
PNL_SHEET_ID = "1dj_Hul3kqJydFp-V2imoFidu5hTMEuncDpkWEJy-gs8"
COACHES_PAY_GID = "1232185270"
PNL_WRITE_URL = "https://script.google.com/macros/s/AKfycbwcpBJ9N6yQODaCMSfJAndguAigdGLUNHVpDNdDliNxhw72hOq2jhMUimD4XWnjC325/exec"
COACHES_PAY_ROW_MAP = {"Chelsea": 2, "Marylebone": 3}  # adjust if your tab layout differs
COACHES_PAY_MONTH_COL = {  # matches existing P&L tabs: Jan=col C(3) ... Dec=col N(14)
...
try:
    ...
    PNL_WRITE_SECRET = userdata.get('PNL_WRITE_SECRET')
except ImportError:
    import os
    PNL_WRITE_SECRET = os.environ.get('PNL_WRITE_SECRET')
...
def push_coaching_cost_to_pnl(studio_costs_df: pd.DataFrame, month_start: str) -> dict:
    """
    Writes each studio's actualized coaching cost into the correct cell of
    the 'Coaches Pay' tab in the P&L sheet, for the month given by month_start.
    Uses COACHES_PAY_ROW_MAP / COACHES_PAY_MONTH_COL above to find the right
    cell — adjust those if your tab's layout differs from row2=Chelsea,
    row3=Marylebone, columns C(Jan)..N(Dec).
    """
    if PNL_WRITE_URL == "PASTE_YOUR_PNL_APPS_SCRIPT_WEBAPP_URL_HERE":
        raise RuntimeError("Set PNL_WRITE_URL at the top of this file once the second Apps Script is deployed.")

    month_name = pd.Timestamp(month_start).strftime("%b")
    if month_name not in COACHES_PAY_MONTH_COL:
        raise RuntimeError(f"Unrecognized month '{month_name}' from month_start='{month_start}'")
    col = COACHES_PAY_MONTH_COL[month_name]

    updates = []
    for _, row in studio_costs_df.iterrows():
        studio = row["studio"]
        if studio not in COACHES_PAY_ROW_MAP:
            print(f"[WARN] No row mapping for studio '{studio}' in COACHES_PAY_ROW_MAP — skipping, not writing anything for it")
            continue
        updates.append({"row": COACHES_PAY_ROW_MAP[studio], "col": col, "value": float(row["coaching_cost"])})

    if not updates:
        print("No studio cost rows matched COACHES_PAY_ROW_MAP — nothing pushed.")
        return {"status": "ok", "updated": 0}

    resp = requests.post(
        PNL_WRITE_URL,
        json={"secret": PNL_WRITE_SECRET, "sheet_name": "Coaches Pay", "updates": updates},
        timeout=30,
    )
    resp.raise_for_status()
    result = resp.json()
    if "error" in result:
        raise RuntimeError(f"Apps Script rejected the write: {result['error']}")
    print(f"Updated {result.get('updated', 0)} cell(s) in Coaches Pay tab for {month_name}.")
    return result
